...
public class ShootingConsistencyTest extends LinearOpMode
{
    ...
    {
        // see https://docs.google.com/document/d/1tyWrXDfMidwYyP_5H4mZyVgaEswhOC35gvdmP-V-5hA
        // /edit#heading=h.61g9ixenznbx
    ...
    {
        // Change my id
        DcMotorEx speedy = hardwareMap.get(DcMotorEx.class, "speedy");
        speedy.setDirection(DcMotorSimple.Direction.FORWARD);
        speedy.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        
        MotorConfigurationType motorConfigurationType = speedy.getMotorType().clone();
        motorConfigurationType.setAchieveableMaxRPMFraction(1.0);
        speedy.setMotorType(motorConfigurationType);
        
        if (RUN_USING_ENCODER)
        {
            speedy.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        }
        else
        {
            speedy.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        }
        
        batteryVoltageSensor = hardwareMap.voltageSensor.iterator().next();
        setPIDFCoefficients(speedy, MOTOR_VELO_PID);
        ...
        {
            setVelocity(speedy, TESTING_SPEED);
            
            printVelocity(speedy, TESTING_SPEED);
    ...
    private void setPIDFCoefficients(DcMotorEx motor, PIDFCoefficients coefficients)
    {
        ...
        if (!DEFAULT_GAINS)
        {
            ...
            motor.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER,
                                      new PIDFCoefficients(coefficients.p,
                                                           coefficients.i,
                                                           coefficients.d,
                                                           coefficients.f * 12 / batteryVoltageSensor
                                                                   .getVoltage()));
        }
        else
        {
